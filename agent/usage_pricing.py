...
_OFFICIAL_DOCS_PRICING: Dict[tuple[str, str], PricingEntry] = {
    # ── Anthropic Claude 5 ───────────────────────────────────────────────
    # Cache multipliers are uniform across the family: 5m write = 1.25x base
    # input, 1h write = 2x, read = 0.1x.  Only the 5m write rate is encoded
    # here since ``cache_write_cost_per_million`` is a single field.
    ...
    # the standard $3/$15 list rate is encoded here so estimates never
    # under-report once the promotion lapses.
    (
        "anthropic",
        "claude-sonnet-5",
    ): PricingEntry(
    ...
    # Fable 5 and Mythos 5 share capabilities, pricing, and API surface.
    (
        "anthropic",
        "claude-fable-5",
    ): PricingEntry(
    ...
    # Google Gemini 3.x.  Rates verified against third-party pricing trackers
    # (July 2026); ai.google.dev was unreachable at capture time, so re-check
    # against the official page before relying on these for billing.
    # 3.1 Pro lists $2/$12 for <=200k-token prompts and $4/$18 beyond that —
    # the short-context tier is encoded here since it covers ordinary traffic.
    (
        "google",
