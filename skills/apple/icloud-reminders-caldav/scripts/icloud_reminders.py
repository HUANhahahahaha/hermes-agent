...
环境变量：
  ...
  REMINDER_LIST         默认写入列表名，默认 ifoon@me.com（--list 可覆盖）
...
# 默认写入的提醒事项列表名；可用环境变量 REMINDER_LIST 覆盖，--list 优先级最高。
DEFAULT_LIST = os.environ.get("REMINDER_LIST", "ifoon@me.com")
...
def _pick_list(principal, name: str | None):
    ...
    target = name or DEFAULT_LIST
    if target:
        for cal in cals:
            if (cal.name or "").strip() == target.strip():
                return cal
        # 显式 --list 没命中才报错；默认列表没命中则静默回退到第一个可写列表。
        if name:
