...
> 跨智能体统一事实索引在仓库根目录 `统一工作记忆.md`。任何 Claude Code、
> Codex 或其他 agent 都先读它，再读本文件；持久的新事实写回同一套记忆。
> 本文件是 `CLAUDE.md` 的跨平台版。两份内容同步维护;冲突时按
> `统一工作记忆.md` 的事实优先级裁决。
...
- **能拿到用户真实导出文件时,绝不重写能跑的部分,只做拼接。**
  `splice_delete_block.py` 保存了拼接方法,但它和另外两份生成器属于不同历史版本;
  不能把仓库脚本当成手机当前 v3 的真相。改手机端前必须重新读取用户当前导出文件,
  核对端点、HTTP 方法和字段后再摘取。快捷指令的「获取URL内容」不把 HTTP 错误码
  当失败,所有循环体都要
  「如果 title 有任何值」保险丝。
